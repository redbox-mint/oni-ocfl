...
class CollectionObject {
  ...
  async addToRepo() {
    ...
    const response =  await checkin(this.collector.repo, this.collector.repoName, this.dir, this.crate, "md5", "ro-crate-metadata.json");
    assert(response, 'Nothing returned to checkin OCFL'); //Stop if cannot add to repo
    console.log(`Wrote crate ${response}`);
