...
const defaults = ROCrate.defaults;
...
const DIALOGUE_SCHEMA_COLUMNS = {
  ...
  class CollectionObject {
    constructor(parent, crateDir){
        ...
        if (crateDir) {
          ...
          const metaPath = path.join(crateDir, defaults.roCrateMetadataID);
    ...
    addDialogueSchema(opts) {
      ...
      */
      ...
      for (let col of opts.columns) {
        ...
        this.crate.addItem(DIALOGUE_SCHEMA_COLUMNS[col]);
      }
    ...
    async addFile(f, subDir) {
          ...
          const srcPath = path.join(this.collector.dataDir, subDir, f["@id"])
  ...
  class Collector {
      constructor(opts) {
          ...
          this.template = this.opts.templateCrateDir;
